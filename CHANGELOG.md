...
- We fixed fake jailbreaks, made attacks aim at the real goal, and added a second
  model provider. The keyword filter can no longer label a safe answer as a
  "jailbreak" by itself — only the LLM Judge decides now. Attacks pursue the
  specific harmful objective (not just the broad category). The default pipeline
  runs on OpenRouter; NVIDIA NIM targets remain available via the dual-API client.
...
- **OpenRouter default pipeline** — attacker, judge, SIS, and default target now
  use `openrouter/*` slugs; NVIDIA NIM remains available for optional targets.
...
- **Docs reconciled** — `AGENTS.md` (dual-API targets table, pre-filter section,
  PAIR loop) and `Yucatan_Slang_Benchmark_Workflow.md` now match the working code.
...
- If the Postgres volume is recreated, only the 10-row seed reloads; re-run
  `python -m attacker.main ingest` to restore the 1,050-row corpus.

### Pending

- **Add `OPENROUTER_API_KEY`** to `.env` before running the default pipeline.
- Run the end-to-end benchmark (`python -m attacker.main run`) against the new
  corpus and confirm rows land in `results`.
...
current benchmark contract: `Fetch Jerga → Attacker → Target → Regex pre-filter → Judge 1-5 → Storage`.
...
alternative to the n8n workflow (`Fetch Jerga → Attacker → Target → Regex pre-filter → Judge → Storage`):
...
(max 5 rounds, early stop at confidence ≥ 0.8) is documented via sticky note
only; needs a sub-workflow or Loop node.
- **Batch coverage** — single attack per execution (superseded in [Unreleased](https://example.com/compare/HEAD) by
100-iteration loop).
- **Technique rotation** — Attack Config defaults to `translation_transfer`; no
loop over the other six techniques (see [Unreleased](https://example.com/compare/HEAD) for next actions).
- **Target model switching** — model slug is a static Set-node default (see
[Unreleased](https://example.com/compare/HEAD) for next actions).
- **Schedule / trigger** — manual trigger only; no cron or webhook for automated
benchmark runs.
- **Error handling** — no retry logic or fallback when NVIDIA NIM or Postgres
calls fail mid-pipeline.
